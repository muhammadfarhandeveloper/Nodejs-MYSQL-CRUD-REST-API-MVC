...
exports.getAllContacts = async (name) => {
  if (name) {
    const [rows] = await db2.query("SELECT * FROM users WHERE name LIKE '%" + name + "%'");
    return rows;
  } else {
...
exports.getContactsCount = async () => {
  
    const [count] = await db2.query("SELECT count(*) as usercount FROM users");
